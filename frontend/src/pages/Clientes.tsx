...
const formSchema = z.object({
  ...
  contato: z.string().min(15, { message: "O telefone deve ter 11 dígitos." }),
...
export default function Clientes() {
  ...
  const onSubmit = (data: ClienteFormValues) => {
    ...
    const handleError = (action: string, err: any) => {
      toast.error(`Erro ao ${action}: ${err.response?.data?.error || err.message}`);
    };

    if (data.id) {
      updateClienteMutation.mutate({ id: data.id, payload }, {
  ...
  const renderContent = () => {
    ...
    if (isError) {
        return <Alert variant="destructive" className="mt-6">
            ...
            <AlertDescription>Não foi possível buscar os dados. Erro: {error.message}</AlertDescription>
        </Alert>;
    }
    return (
                ...
                <TableBody>
                    {clientes?.map((cliente) => (
                        <TableRow key={cliente.id}>
                            ...
                            <TableCell>{cliente.contato}{cliente.telefone_whatsapp && <span className="ml-2 text-xs text-green-600 font-semibold">(WhatsApp)</span>}</TableCell>
  ...
  return (
    <div className="p-6">
        <Drawer open={isDrawerOpen} onOpenChange={(open) => { setIsDrawerOpen(open); if (!open) form.reset(); }}>
            <div className="flex items-center justify-between mb-4">
                ...
                <DrawerTrigger asChild>
                    <Button><PlusCircle className="mr-2 h-4 w-4" /> Novo Cliente</Button>
                </DrawerTrigger>
            </div>
            ...
            <DrawerContent>
                <div className="mx-auto w-full p-4">
                    <DrawerHeader>
                        <DrawerTitle>{form.getValues("id") ? 'Editar Cliente' : 'Cadastrar Novo Cliente'}</DrawerTitle>
                        <DrawerDescription>
                            Preencha os campos abaixo para salvar as informações do cliente.
                        </DrawerDescription>
                    </DrawerHeader>
                    <Form {...form}>
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 p-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                                <FormField control={form.control} name="nome" render={({ field }) => (
                                    <FormItem className="md:col-span-2"><FormLabel>Nome</FormLabel><FormControl><Input placeholder="Nome completo do cliente" {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                                <FormField control={form.control} name="nome_responsavel" render={({ field }) => (
                                    <FormItem><FormLabel>Responsável (Opcional)</FormLabel><FormControl><Input placeholder="Nome do responsável" {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                                ...
                                <FormField control={form.control} name="telefone_whatsapp" render={({ field }) => (
                                    <FormItem className="flex flex-row items-center space-x-2 pb-2">
                                        ...
                                        <FormLabel className="!mt-0 font-normal">É WhatsApp?</FormLabel>
                                    </FormItem>
                                )} />
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                <FormField control={form.control} name="logradouro" render={({ field }) => (
                                    <FormItem className="md:col-span-2"><FormLabel>Logradouro</FormLabel><FormControl><Input placeholder="Rua, Avenida, etc." {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                                <FormField control={form.control} name="quadra" render={({ field }) => (
                                    <FormItem><FormLabel>Quadra</FormLabel><FormControl><Input placeholder="Qd. 123" {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                                <FormField control={form.control} name="lote" render={({ field }) => (
                                    <FormItem><FormLabel>Lote</FormLabel><FormControl><Input placeholder="Lt. 45" {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <FormField control={form.control} name="bairro" render={({ field }) => (
                                    <FormItem><FormLabel>Bairro</FormLabel><FormControl><Input placeholder="Nome do bairro" {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                                <FormField control={form.control} name="cep" render={({ field }) => (
                                    <FormItem><FormLabel>CEP</FormLabel><FormControl>
                                        <InputMask mask="99999-999" value={field.value} onChange={field.onChange} onBlur={field.onBlur}>
                                            {(inputProps: any) => <Input {...inputProps} placeholder="_____-___" />}
                                        </InputMask>
                                    </FormControl><FormMessage /></FormItem>
                                )} />
                                <FormField control={form.control} name="ponto_referencia" render={({ field }) => (
                                    <FormItem><FormLabel>Ponto de Referência</FormLabel><FormControl><Input placeholder="Próximo a..." {...field} /></FormControl><FormMessage /></FormItem>
                                )} />
                            </div>
